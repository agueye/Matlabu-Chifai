  ...
  def create
    ...
    respond_to do |format|
      if @patient_allergy.save
        ...
        APP_LOGGER_LOG.info "ALLERGY CREATED - for PATIENT ID " + 
          @patient_allergy.patient[:medical_record_number].to_s + " by USER " + self.current_user[:login]
  ...
  def update
    ...
    respond_to do |format|
      if @patient_allergy.update_attributes(params[:patient_allergy])
        ...
        APP_LOGGER_LOG.info "ALLERGY UPDATED - for PATIENT ID " + 
